package com.ochat.android.ui.debug
...
import com.ochat.android.services.meshgraph.MeshGraphService
...
fun MeshTopologySection(
    ...
    val wifiAwareConnected by com.ochat.android.wifiaware.WifiAwareController.connectedPeers.collectAsState()
...
fun DebugSettingsSheet(
    ...
    val wifiAwareSupported by com.ochat.android.wifiaware.WifiAwareController.supported.collectAsState()
    val wifiAwareAvailable by com.ochat.android.wifiaware.WifiAwareController.available.collectAsState()
    val wifiAwareSupportStatus by com.ochat.android.wifiaware.WifiAwareController.supportStatus.collectAsState()
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.ochat.android.ui.debug.ConnectedDevice(
                ...
                try {
                    val ctrl = com.ochat.android.wifiaware.WifiAwareController
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        val running by com.ochat.android.wifiaware.WifiAwareController.running.collectAsState()
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            AssistChip(
                                onClick = { com.ochat.android.wifiaware.WifiAwareController.getService()?.sendBroadcastAnnounce() },
                                enabled = running,
                                label = { Text("Announce") }
                            )
                        }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.ochat.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.ochat.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
