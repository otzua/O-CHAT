...
class PowerManager(private val context: Context) : LifecycleEventObserver {
    ...
    companion object {
        ...
        // Battery thresholds
        private const val CRITICAL_BATTERY = com.ochat.android.util.AppConstants.Power.CRITICAL_BATTERY_PERCENT
        private const val LOW_BATTERY = com.ochat.android.util.AppConstants.Power.LOW_BATTERY_PERCENT
        private const val MEDIUM_BATTERY = com.ochat.android.util.AppConstants.Power.MEDIUM_BATTERY_PERCENT
        
        // Scan duty cycle periods (ms)
        private const val SCAN_ON_DURATION_NORMAL = com.ochat.android.util.AppConstants.Power.SCAN_ON_DURATION_NORMAL_MS    // 8 seconds on
        private const val SCAN_OFF_DURATION_NORMAL = com.ochat.android.util.AppConstants.Power.SCAN_OFF_DURATION_NORMAL_MS   // 2 seconds off
        private const val SCAN_ON_DURATION_POWER_SAVE = com.ochat.android.util.AppConstants.Power.SCAN_ON_DURATION_POWER_SAVE_MS    // 2 seconds on
        private const val SCAN_OFF_DURATION_POWER_SAVE = com.ochat.android.util.AppConstants.Power.SCAN_OFF_DURATION_POWER_SAVE_MS  // 8 seconds off
        private const val SCAN_ON_DURATION_ULTRA_LOW = com.ochat.android.util.AppConstants.Power.SCAN_ON_DURATION_ULTRA_LOW_MS      // 1 second on
        private const val SCAN_OFF_DURATION_ULTRA_LOW = com.ochat.android.util.AppConstants.Power.SCAN_OFF_DURATION_ULTRA_LOW_MS   // 10 seconds off
        
        // Connection limits
        private const val MAX_CONNECTIONS_NORMAL = com.ochat.android.util.AppConstants.Power.MAX_CONNECTIONS_NORMAL
        private const val MAX_CONNECTIONS_POWER_SAVE = com.ochat.android.util.AppConstants.Power.MAX_CONNECTIONS_POWER_SAVE
        private const val MAX_CONNECTIONS_ULTRA_LOW = com.ochat.android.util.AppConstants.Power.MAX_CONNECTIONS_ULTRA_LOW
    }
    ...
    private fun updatePowerMode() {
        ...
        if (newMode != currentMode) {
            ...
            if (shouldUseDutyCycle()) {
                ...
                //
                // stopDutyCycle() only cancels the timer. If the cycle happened to be in its
                // OFF phase - 28 of every 30 seconds in POWER_SAVER, so almost always - the
                // scanner was left off and the timer that would have switched it back on is
                // now cancelled, so scanning never resumes. Enable it explicitly.
                stopDutyCycle()
