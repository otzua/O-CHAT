...
class BluetoothConnectionManager(
     ...
     * whether the behaviour actually changed. PowerManager updates its own currentMode before
     * invoking the callback, so it cannot be asked what the previous state was.
     */
    ...
    private fun isGattServerEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.ochat.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }

    private fun isGattClientEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.ochat.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    init {
        ...
        try {
            val dbg = com.ochat.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            //
            // This must compare against the previous mode remembered here. PowerManager sets
            // its currentMode before invoking this callback, so querying shouldUseDutyCycle()
            // twice around the advertising restart returned the same value every time and the
            // scan was never restarted on a mode change.
